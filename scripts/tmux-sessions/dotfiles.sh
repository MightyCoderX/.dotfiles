...
if ! tmux list-sessions | grep -q "$session"; then
	...
	tmux new-session -d -c "$root_dir" -s "$session" "cd $root_dir/.config; nvim ."
	tmux rename-window -t "$session:$window_index" '.config'

	_new_window 'hypr*' "cd $root_dir/.config/hypr; nvim ."
	...
	_new_window 'shell' "cd $root_dir/shell; nvim ."
	_new_window 'fish' "cd $root_dir/.config/fish; nvim ."
	_new_window 'nvim' "cd $root_dir/.config/nvim; nvim ."

	_new_window 'scripts' "cd $root_dir/scripts; nvim ."
